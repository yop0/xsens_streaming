...
/*! \class ScaleDatagram
  ...
  This packet contains scaling information about the character. This information is only useful for MVN Studio, since it
  only applies to a mesh of a specific size. However, there may be a way to put this information to use for other
  meshes.
...
ScaleDatagram::ScaleDatagram() : Datagram()
{
...
/*! Deserialize the scale packets
  The Scale packets are divided in parts, to avoid exceed the 1500 byte packet size limit. For a normal biped this
  results in a total of 5 packets. To deserialize the packets whe should follow this rules:
...
void ScaleDatagram::deserializeData(Streamer & inputStreamer)
{
  ...
  for(int i = 0; i < numberOfSegments; i++)
  {
    ...
    for(int k = 0; k < 3; k++) streamer->read(static_cast<float &>(nullPosDef.pos[k]));
  ...
  */
  if(numberOfSegments == 0)
  {
    ...
    for(int i = 0; i < numberOfPoints; i++)
    {
      ...
      for(int k = 0; k < 3; k++) streamer->read(static_cast<float &>(pointDef.pos[k]));
