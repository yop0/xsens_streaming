...
/*! Constructor */
TrackerKinematicsDatagram::TrackerKinematicsDatagram() : Datagram()
{
...
void TrackerKinematicsDatagram::deserializeData(Streamer & inputStreamer)
{
  ...
  for(int i = 0; i < dataCount(); i++)
  {
    ...
    for(int k = 0; k < 4; k++) streamer->read(static_cast<float &>(kin.sens_rot[k]));

    // Store the Sensor free acceleration in a Vector -> 12 byte	(3 x 4 byte)
    for(int k = 0; k < 3; k++) streamer->read(static_cast<float &>(kin.sen_freeAcc[k]));

    // Store the  Sensor Acceleration in a Vector -> 12 byte	(3 x 4 byte)
    for(int k = 0; k < 3; k++) streamer->read(static_cast<float &>(kin.sen_acc[k]));

    // Store the Sensor gyroscope in a Vector -> 12 byte	(3 x 4 byte)
    for(int k = 0; k < 3; k++) streamer->read(static_cast<float &>(kin.sen_gyr[k]));

    // Store the Sensor magnetometer a Vector -> 12 byte	(3 x 4 byte)
    for(int k = 0; k < 3; k++) streamer->read(static_cast<float &>(kin.sen_mag[k]));
