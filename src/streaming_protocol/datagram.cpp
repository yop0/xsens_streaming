...
Datagram::Datagram()
: m_header("MXTP00"), m_avatarId(0), m_dgramCounter(0x80), m_dataCount(0), m_frameTime(0), m_sampleCounter(0)

  ,
  m_bodySegmentCount(23), m_propCount(0), m_fingerTrackingSegmentCount(0)
...
{
  std::cout << "\n************* " << decode(static_cast<StreamingProtocol>(messageType())) << "(" << (int)avatarId()
            << ")"
            << " [" << (int)datagramCounter() << "] " << getDataSize() << " bytes *************"

            << "\nNumber of items: " << (int)dataCount() << "\tSample counter: " << sampleCounter()
            << "\tFrame Time: " << frameTime()

            << "\nBody Segment Count: " << bodySegmentCount() << "\tProp Count: " << propCount()
            << "\tFingerTS Count: " << fingerTrackingSegmentCount() << std::endl;
}

/*! Return the avatar ID

  MVN Studio now supports multiple avatars in one viewport. This byte specifies to which avatar the data belongs. In a
  single-avatar setup this value will always be 0. In multi-avatar cases, they will usually be incremental. However,
  especially during live streaming, one of the avatars may disconnect and stop sending data while others will continue
  so the receiver should be able to handle this. Each avatar will send its own full packet.
...
/*! Return the amount of data items in this datagram
  ...
  The number of items is stored as an 8-bit unsigned integer value. This number indicates the number of segments or
  points that are contained in the packet. Note that this number is not necessarily equal to the total number of sensors
  that were being captured at the sampling instance if the motion capture data was split up into several datagrams. This
  number may instead be used to verify that the entire UDP datagram has been fully received by calculating the expected
  size of the datagram and comparing it to the actual size of the datagram.
*/
...
/*! The sample counter is a 32-bit unsigned integer value which is incremented by one each time a new set of motion
 * sensor data is sampled and sent away. Note that the sample counter is not to be interpreted as a time code, since the
 * sender may skip frames. */
...
The size of a UDP datagram is usually limited by the MTU (maximum transmission unit, approx. 1500 bytes) of the
underlying Ethernet network. In nearly all cases the entire motion data that was collected at one sampling instance will
fit into a single UDP datagram. However, if the amount of motion data becomes too large then the data is split up into
several datagrams.
...
The most significant bit of the datagram counter byte is used to signal that this datagram is the last one belonging to
that sampling instance. For example, if motion data is split up into three datagrams then their datagram counters will
have the values 0, 1 and 0x82 (hexadecimal). If all data fits into one UDP datagram (the usual case) then the datagram
counter will be equal to 0x80 (hexadecimal).

The sample counter mentioned above can be used to identify which datagrams belong to the same sampling instance because
they must all carry the same sample counter value but different datagram counters. This also means that the combination
of sample counter and datagram counter values is unique for each UDP datagram containing (part of the) motion data.
...
/*! The frametime associated with this datagram
  ...
  MVN Studio contains a clock which starts running at the start of a recording. The clock measures the elapsed time in
  milliseconds. Whenever new captured data is sampled the current value of the clock is sampled as well and is stored
  inside the datagram(s) as a 32-bit unsigned integer value representing a time code.
*/
