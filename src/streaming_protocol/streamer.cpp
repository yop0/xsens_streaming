...
Streamer::Streamer(const XsByteArray & arr) : m_offset(0)
{
...
void Streamer::read(int32_t & destination)
{
  ...
  if(m_endianess == Little_Endian)
    destination =
        (byte_array[0] << 24) | ((byte_array[1] & 0xff) << 16) | ((byte_array[2] & 0xff) << 8) | (byte_array[3] & 0xff);
  else
    destination =
        (byte_array[3] << 24) | ((byte_array[2] & 0xff) << 16) | ((byte_array[1] & 0xff) << 8) | (byte_array[0] & 0xff);
